...
- [ ] **Task:** Implementar los métodos `createCoin`, `updateCoin` y `deleteCoin` en la interfaz `CoinService` y su implementación `CoinServiceImpl`.
- [ ] **Task:** Desarrollar pruebas unitarias con `Mockito` en `CoinServiceTest` para los métodos de guardado, actualización y borrado de monedas.
